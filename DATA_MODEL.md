...
- After importing each file into Power BI, add a column named, `Pest` identifying the pest the file belongs to.
...
- For each entity type, append all pest‑specific files into a single consolidated table:
...
- Relationships in the model:
  ...
  - `Masters[Pest]` → `PestTable[Pests]` (many‑to‑one, to `PestTable`)

### PestTable (single‑column lookup)

- A dedicated lookup table `PestTable` with a single column `Pests` listing all active pests.
